...
function form_validate4() {
	if (facebook.value.length <= 0) {
		...
		alert("facebook didn't entered");
		return false;
	}
	if (twitter.value.length <= 0) {
		twitter.style.borderColor = 'red';
		alert("twitter didn't entered");
		return false;
	}
...
function displayDetail(){
    ...
    //var dob =Date.now() - (document.getElementById("dob").value).getTime();
	...
	//var userinput = document.getElementById("DOB").value;
	document.getElementById("d-dob").innerText =userinput;
